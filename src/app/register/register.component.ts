...
import { UsernameGeneratorComponent } from 'src/app/ai/username-generator/username-generator.component'; 
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.loading = false;
           ...
           // ** Exemple basé sur un message d'erreur spécifique ou un statut 409 Conflict (ADAPTEZ cette condition !!!) **
           // Si votre backend renvoie 409 CONFLICT pour "nom pris":
           // if (err.status === 409) { ... }
           // Si votre backend renvoie un message spécifique dans le corps pour "nom pris":
           // if (errorMessage.includes('Nom d\'utilisateur déjà existant') || errorMessage.includes('Username already taken') /* ou un code spécifique */) {
